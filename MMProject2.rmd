...
``` {r DataImport, include = T, results = F}
loadPkg('tidyverse')
...
For the section, we will look at the Feature Selection in Linear Models.  We will continue using the dataset for the 1960s and 2000s songs.  We will first eliminate any variables that we believe are useless for this section.  We only have to remove the variable labeled as target and mode.

```{r removal, include=T}
# cleaned dataset
data1960s <- data.frame(read.csv("dataset-of-60s.csv"))
data2000s <- data.frame(read.csv("dataset-of-00s.csv"))

data1960_1 = data1960s[,c('track', 'artist', 'danceability', 'energy', 'key', 'liveness', 'valence', 'tempo')] 
data2000_1 = data2000s[,c('track', 'artist', 'danceability', 'energy', 'key', 'liveness', 'valence', 'tempo')]
data1960_1
data2000_1

data1960_1$danceability <- as.integer(data1960_1$danceability)
data1960_1$energy <- as.integer(data1960_1$energy)
data1960_1$key <- as.integer(data1960_1$key)
data1960_1$liveness <- as.integer(data1960_1$liveness)
data1960_1$valence <- as.integer(data1960_1$valence)
data1960_1$tempo <- as.integer(data1960_1$tempo)
data1960_1

data2000_1$danceability <- as.integer(data2000_1$danceability)
data2000_1$energy <- as.integer(data2000_1$energy)
data2000_1$key <- as.integer(data2000_1$key)
data2000_1$liveness <- as.integer(data2000_1$liveness)
data2000_1$valence <- as.integer(data2000_1$valence)
data2000_1$tempo <- as.integer(data2000_1$tempo)
data2000_1
...
###Exhaustive search
We are now going to use the exhaustive search to get the best results to find out which variables will create the best model to show which factors affects whether a song is popular or not in both 1960s and 2000s.

```{r exhaustive search, results=T}
#This is essentially best fit 
#install.packages('leaps')
#install.packages('cars')
loadPkg('leaps')
#loadPkg(cars)
#reg.best8 <- regsubsets(key~. , data = data2000_1, nbest = 2, method = "exhaustive")
#plot(reg.best8, scale = "adjr2", main = "Adjusted R^2")
#plot(reg.best8, scale = "r2", main = "R^2")
#plot(reg.best8, scale = "bic", main = "BIC")
#plot(reg.best8, scale = "Cp", main = "Cp")
#summary(reg.best8)


reg.2000 <- regsubsets(key~. , data = data1960_1, nbest = 2, method = "exhaustive")
plot(reg.2000, scale = "adjr2", main = "Adjusted R^2")
plot(reg.2000, scale = "r2", main = "R^2")
plot(reg.2000, scale = "bic", main = "BIC")
plot(reg.2000, scale = "Cp", main = "Cp")
summary(reg.2000)
```
