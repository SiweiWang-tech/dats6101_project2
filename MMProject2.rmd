...
```{r mode_TTest_Gude2}
# Perform tst
ttest_mode60 = t.test((as.integer(subset(data1960,target ==0)$mode)-1), (as.integer(subset(data1960,target ==1)$mode)-1))
ttest_mode60

ttest_mode00 = t.test((as.integer(subset(data2000,target ==0)$mode)-1), (as.integer(subset(data2000,target ==1)$mode)-1))
ttest_mode00
```
...
The purpose of this study is to analyse the affect of music attributes, such as tempo, key, mode, etc, on the popularity of songs with respect to time via the Spotify 

The purpose of this study was to analyze the effect of genetic, wealth, and fitness factors, specifically correlating with ethnicity, gender, age, household income, and veteran status of interviewed adult (ages 18 and older) from the NHANES from 2005 and 2006, on human blood pressure in order to inform follow-on research and development efforts.  From the tests and results conducted and reported herein this study, it has been shown that there is a clear, statistically significant affect on blood pressure from all five factors (ethnicity, gender, age, household income, and veteran status).  Additional testing and study may be warranted to flesh out the exact mechanisms by which each of these factors may or may not affect blood pressure (since correlation does not imply causation).  Additionally from the results, the effect from ethnicity and gender on systolic pressure had the most significant effects on average blood pressures compared to the rest of the factors, and more research may be directed in understanding the correlation.
